# ふるさと納税計算ソフト
所得金額などを入力し、ふるさと納税の自己負担額を計算します。
自己負担額が少なくなるようなふるさと納税額を計算できます。

株式や不動産の譲渡所得、所得控除、住宅ローン控除などにも対応しています。

## furusato2026.jar

親族などの入力内容を変更し、扶養控除などを自動判定にしました。

繰越損失などを考慮した税額計算にしました。

分離譲渡所得の特別控除に対応しました。

雑損控除の入力内容を変更しました。

version8.00

version8.01　分離譲渡所得の特別控除の誤り修正、他、多数修正

version8.02　生命保険料控除の誤り修正

version8.03　障害者控除の誤り修正

version8.10　ワンストップ特例の計算を追加、住民税の計算における所得税基礎控除額差額の表示

## furusato2026_v702.jar

以前のfurusato2026.jarです。

バグ対応などしない予定ですので、今後は上の`furusato2026.jar`をお使いください。

~~ダブルクリックで起動しません。コマンドプロンプトから、`java -jar furusato2026.jar`で起動します。~~

２０２６年版

給与所得控除、基礎控除（所得税）、寄附金税額控除（住民税）、住宅借入金等特別税額控除（住民税）などの改正に対応しています。

version7.00 住民税調整控除額の計算誤り修正、プログラム構成の大幅改変

version7.01 所得税の所得控除の計算誤り修正

version7.02 配偶者の住民税の所得金額計算、医療費控除、計算過程表示の修正

## furusato2025.jar

２０２５年版　~~令和７年度税制改正与党大綱に対応しています。実際の改正とは異なっている可能性があります。~~

 基礎控除（所得税）、 給与所得控除（所得税、住民税）、 特定親族特別控除（所得税、住民税）~~、生命保険料控除（所得税）~~

 特定親族特別控除は、3人までです。各特定親族の合計所得金額を入力してください。

 生命保険料控除については、「２３歳未満扶養親族」欄を追加しています。令和7年では生命保険料控除は改正されていませんので、この欄は本来は不要ですが、残してあります。

version5.00~~0.50~~ 税制改正大綱版

version5.01~~0.51~~ 生命保険料控除の計算誤り修正

version5.02　修正案版

version5.03　改正条文版　生命保険料控除の誤り修正、住民税の配偶者控除の判定誤り修正、ファイル読込機能の修正

version5.04　給与所得がマイナスになる点を修正

version5.05　勤労学生控除の所得条件を８５万円に修正（なお、給与所得等以外の所得の条件などのチェックはしていません）

version5.06　令和８年度税制改正（所得税の基礎控除額の調整のみ対応、住宅ローン控除の調整は未対応）

## furusato2020_v405.jar

２０２０年版となっていますが、２０２２年でも使えます。
税制改正に影響しない場合には、２０２３年でも使えます。

なお、２０２３年以後は、上場株式の配当等や譲渡等については、所得税と住民税が同じ課税方法になる点にご注意ください。
